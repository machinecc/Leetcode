...
public:
    ...
    int ladderLength(string beginWord, string endWord, unordered_set<string>& wordDict) {
    	...
    	if (wordDict.size() == 0) {
    		if (hdistance(beginWord, endWord) <= 1)
    			return 1;
    		else
    			return 0;
    	}
    	...
    	copy(wordDict.begin(), wordDict.end(), back_inserter(words));
		...
		for (int i = 0; i < m; ++ i) {
			d[i][i] = 0;
			for (int j = i + 1; j < m; ++ j) {
				if (hdistance(words[i], words[j]) == 1)
					d[i][j] = 1;
				else
					d[i][j] = word_len + 1;
		...
		for (int i = 0; i < m; ++ i) {
			distance[i] = word_len + 1;
			visited[i] = false;
		}


		for (int r = 0; r < m; ++ r) {
			int min_path = word_len;
			int i = -1;
			...
			if (r == 0) {
				...
				for (int j = 0; j < m; ++ j)
					if (visited[j] == false && distance[j] < min_path) {
						min_path = distance[j];
						i = j;
					}
			}
			...
			for (int j = 0; j < m; ++ j) {
				if (visited[j] == false && d[i][j] == 1 && distance[i] + d[i][j] < distance[j])
		...
		for (int i = 0; i < m; ++ i)
			cout << words[i] << " -> " << endWord << ":   " << distance[i] << endl;
		cout << endl << endl;
		...
		for (int i = 0; i < m; ++ i)
			if (hdistance(beginWord, words[i]) == 1) {
				if (min_val == -1 || distance[i] < min_val)
					min_val = distance[i];
			}
    ...
    int hdistance(const string& s1, const string& s2) {
    	int n = 0;
